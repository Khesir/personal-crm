...
const _kKeepTrackFields = [
  _EnvField('KEEP_TRACK_BASE_URL', 'Backend URL', 'http://localhost:3000', false),
  _EnvField('CRM_SECRET', 'CRM Secret', 'your-crm-secret', true),
  _EnvField('VERCEL_TOKEN', 'Vercel Token', 'your-vercel-api-token', true),
  _EnvField('VERCEL_PROJECT_ID', 'Vercel Project ID', 'prj_...', false),
  _EnvField('VERCEL_TEAM_ID', 'Vercel Team ID (optional)', 'team_...', false),
  _EnvField('LEMON_SQUEEZY_API_KEY', 'LemonSqueezy API Key', 'your-api-key', true),
];
...
class KeepTrackSettingsSection extends StatefulWidget {
  const KeepTrackSettingsSection({super.key});

  @override
  State<KeepTrackSettingsSection> createState() => _KeepTrackSettingsSectionState();
}

class _KeepTrackSettingsSectionState extends State<KeepTrackSettingsSection> {
  ...
  void initState() {
    super.initState();
    _ctrls = {
      for (final f in _kKeepTrackFields)
        f.key: TextEditingController(text: dotenv.env[f.key] ?? ''),
    };
    for (final f in _kKeepTrackFields) {
  ...
  Future<void> _save() async {
    ...
    for (final f in _kKeepTrackFields) {
      ...
      if (value.isEmpty) {
        await prefs.remove('$kSettingsPrefix${f.key}');
      } else {
        await prefs.setString('$kSettingsPrefix${f.key}', value);
        dotenv.env[f.key] = value;
      }
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Text('Environment configuration for Keep Track.', style: AppStyling.bodySm),
          const SizedBox(height: AppStyling.spaceXxl),
          ...(_kKeepTrackFields.map((f) => Padding(
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: _saved ? AppColors.success : AppColors.accentKeepTrack,
