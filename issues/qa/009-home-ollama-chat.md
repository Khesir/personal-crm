...
- Added `lib/features/home/` with `ChatConversation`/`ChatMessage` models, `OllamaRepository`
  (`/api/tags`, NDJSON-streamed `/api/chat`), `ChatConversationsRepository` (JSON in
  `shared_preferences`), and `ChatController`. Built `HomeSidebarSection` (conversation list + new chat)
  and `HomeChatSection` (header + model switcher, message list with markdown/code-block rendering,
  composer, empty state with suggested prompts), wired into the shell's Home tab/sidebar.
...
- `screen-home.jsx` / `Dev Command Center.html` mockup referenced by the issue were not found in the repo;
  UI was based on the PRD and this issue's text description plus the existing Codex theme tokens.
