...
### 2026-06-19
Implemented `tools/research.py` with `trigger_research` (returns UUID immediately, fires async pipeline via `asyncio.run_coroutine_threadsafe` / `ensure_future`) and `manage_research` (status/read actions with in-memory job dict + JSON file fallback). Pipeline: LLM decompose → `_web_search` per sub-question → `_web_fetch` top URL → LLM synthesize → save to `%APPDATA%\Avyn\agent\research\{id}.json`. SearXNG failures skip gracefully without crashing. Wired `set_provider` and `set_loop` into `main.py` startup and `/config` handler. Created `test_research.py` with 4 tests: returns-id, running-status, done-read, graceful-searxng-failure.
