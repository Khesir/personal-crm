...
### 2026-06-19
Created `agent/build.spec` (PyInstaller one-file spec with uvicorn, fastapi, chromadb hidden imports), `agent/build.ps1` (build script, installs pyinstaller if absent, `-Clean` flag), and updated `agent/README.md` with build instructions and manual acceptance gate. Added `agent/dist/` and `agent/build/` to root `.gitignore`. Automated tests not applicable — acceptance gate requires manual run on a clean Windows machine without Python.
