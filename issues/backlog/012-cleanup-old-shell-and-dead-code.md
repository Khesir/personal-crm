...
- [ ] No references to `accentPortfolio`, `accentKeepTrack`, `accentTimeTracker`, `accentAriConnect`, or
      `accentMinecraft` remain.
- [ ] The `keep_track` feature folder contains only what 007 moved out of it, or is removed entirely if
      everything was moved.
- [ ] Build and analyzer run clean with no unused-import/dead-code warnings introduced by the pivot.
