...
- [ ] ChromaDB is initialised at `%APPDATA%\Avyn\agent\chroma\` on server startup
- [ ] After each loop run, the assistant response is embedded and stored in ChromaDB
- [ ] Before each loop run, top-k semantically similar past responses are retrieved
